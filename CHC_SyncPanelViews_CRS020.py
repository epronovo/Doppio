# CHC_SyncPanelViews.py
"""
CHC_SyncPanelViews.py
---------------------
Syncs panel view column configurations from the local doppio.db (SOURCE)
to a DESTINATION tenant.

Variant of MIG_SyncPanelViews.py: the SOURCE EXPORTMI.Select call is
replaced by a query against the CSYSPV table loaded by chc_spv.py.
Field names match on the C9xxxx column names; fields the table does not
have (C9LUFN, C9LUVE, C9MCRT) are blank.

Steps
-----
1.  Read CSYSPV from doppio.db (SOURCE) into memory.
...
SOURCE_FIELDS   = [f.strip() for f in _EXPORTMI_FIELDS.split(",")]
...
def fetch_panel_views_db(db_path: str, pgnm_filter: str) -> list[dict]:
    """
    SOURCE replacement for fetch_panel_views(): reads panel-view rows from
    the local CSYSPV table (loaded by chc_spv.py) and returns the same
    structure — one dict per row keyed by the C9xxxx field names.

    Fields requested from EXPORTMI that do not exist in the table
    (C9LUFN, C9LUVE, C9MCRT) are returned blank.  NULL cells are blank.
    C9PARA is preserved as-is (fixed-width positional string); all other
    values are stripped strings, matching the EXPORTMI parsing above.
    """
    conn = sqlite3.connect(db_path)
    try:
        table_cols = {r[1] for r in conn.execute(f"PRAGMA table_info({SOURCE_TABLE})")}
        if not table_cols:
            raise RuntimeError(
                f"Table {SOURCE_TABLE} not found in {db_path} — run chc_spv.py first."
            )

        missing = [f for f in SOURCE_FIELDS if f not in table_cols]
        if missing:
            print(f"  ⚠️   Not in {SOURCE_TABLE}, returned blank: {', '.join(missing)}")

        select_cols = ", ".join(
            f'"{f}"' if f in table_cols else f"'' AS \"{f}\""
            for f in SOURCE_FIELDS
        )
        sql = (
            f"SELECT {select_cols} FROM {SOURCE_TABLE} "
            f"WHERE C9CONO = 1 AND C9PAVR IS NOT NULL AND TRIM(C9PAVR) <> '' AND C9RESP is null"
        )
        params: list = []
        if pgnm_filter:
            sql += " AND C9PGNM = ?"
    ...
    for raw in rows:
        row: dict = {}
        for field, val in zip(SOURCE_FIELDS, raw):
            if val is None:
                row[field] = ""
            elif field == "C9PARA":
                row[field] = str(val)          # fixed-width — keep positions intact
            else:
                row[field] = str(val).strip()
        parsed.append(row)

    print(f"  ✅  {len(parsed)} panel-view records read from {SOURCE_TABLE} (SOURCE).")
    return parsed
...
PARA_CMP_LEN = 300   # only the first 300 characters are meaningful for comparison
...
# The first segment of C9PARA is a list of subfile column names, one per
# 10-char slot, left-packed with no gaps.  The list ends at the first fully
# blank slot.  Everything after that (flags, attribute codes, widths) is
# packed per-field, so inserting/removing a column shifts all of it — that
# shows up as many scattered ranges in para_diff_summary even though the
# real change is "these columns were added/removed".
FIELD_SLOT_LEN = 10


def decode_field_names(c9para: str) -> list[str]:
    """Return the ordered list of subfile column names encoded in C9PARA."""
    names: list[str] = []
    for i in range(0, len(c9para), FIELD_SLOT_LEN):
        slot = c9para[i : i + FIELD_SLOT_LEN]
        if not slot.strip():
            break
        names.append(slot.strip())
    return names


def describe_para_diff(src: str, dst: str) -> str:
    """
    Human-readable summary of a C9PARA difference: reports column-list
    changes (added/removed/reordered fields) separately from the remaining
    positional differences (flags/attribute codes/widths) that follow the
    column list, since a single column add/remove otherwise looks like
    dozens of unrelated diff ranges.
    """
    src_names = decode_field_names(src)
    dst_names = decode_field_names(dst)

    parts: list[str] = []
    if src_names != dst_names:
        added   = [f for f in dst_names if f not in src_names]
        removed = [f for f in src_names if f not in dst_names]
        field_bits = []
        if added:
            field_bits.append("+" + " +".join(added))
        if removed:
            field_bits.append("-" + " -".join(removed))
        if not added and not removed:
            field_bits.append("reordered")
        parts.append(f"fields: {' '.join(field_bits)}")

        # Compare everything after the (longer) column list separately so
        # the shift caused by the column change doesn't drown out the rest.
        tail_start = max(len(src_names), len(dst_names)) * FIELD_SLOT_LEN
        tail_diff = para_diff_summary(src[tail_start:], dst[tail_start:])
        if tail_diff != "identical":
            parts.append(f"other: {tail_diff}")
    else:
        parts.append(para_diff_summary(src, dst))
...
def sync_panel_views() -> None:
    ...
    # ------------------------------------------------------------------ #
    # Step 1 – SOURCE: local doppio.db (CSYSPV)                           #
    # ------------------------------------------------------------------ #
    print(f"\n🔍  Step 1 – Reading {SOURCE_TABLE} from {DEFAULT_DB_PATH} (SOURCE) …")
    source_views = fetch_panel_views_db(DEFAULT_DB_PATH, pgnm_filter)
    ...
    # ------------------------------------------------------------------ #
    print(f"\n{'═' * 60}")
    print(f"  CHC_SyncPanelViews – Run Summary")
    print(f"{'═' * 60}")
