# CHC_SyncSortingOptions.py
"""
CHC_SyncSortingOptions.py
-------------------
Syncs sort options from the local doppio.db (SOURCE) to a DESTINATION tenant.

Variant of MIG_SyncSortingOptions.py: the SOURCE CRS021MI.LstSrtOpt call is
replaced by a query against the CSYSOR table loaded by chc_spv.py.
API field names are derived from the last 4 characters of each CIxxxx
column (CIFILE→FILE, CISOPT→SOPT, CIKEY1→KEY1, CIKE10→KE10, …).
NULL / missing values are blank.  Audit columns (RGDT, RGTM, LMDT, CHNO,
CHID, LMTS) are excluded because LstSrtOpt does not return them.

Steps
-----
1.  Read CSYSOR from doppio.db (SOURCE) → store in memory.
...
Usage:
    python CHC_SyncSortingOptions.py
"""
...
# Audit columns not returned by CRS021MI.LstSrtOpt — excluded from records
AUDIT_FIELDS = frozenset({"RGDT", "RGTM", "LMDT", "CHNO", "CHID", "LMTS"})
...
def pad_sopt(sopt: str) -> str:
    ...
    stored as 0 and '01' as 1 in CSYSOR.  Zero-pad pure digits back to
...
def list_all_srt_opts_db(db_path: str) -> list[dict]:
    """
    SOURCE replacement for list_all_srt_opts(): reads every row from the
    local CSYSOR table (loaded by chc_spv.py) and returns records shaped
    like CRS021MI.LstSrtOpt output.

    Field names are the last 4 characters of each CIxxxx column
    (CIFILE→FILE, CISOPT→SOPT, CIKEY1→KEY1, CIKE10→KE10, …).
    NULL values are blank.  Audit columns are excluded.
    Numeric SOPT values are zero-padded back to 2 characters.
    """
    ...
    try:
        ...
        keep = [(c, c[-4:]) for c in cols if c[-4:] not in AUDIT_FIELDS]
        select_cols = ", ".join(f'"{c}"' for c, _ in keep)
    ...
    for raw in rows:
        rec: dict = {}
        for (_, api_name), val in zip(keep, raw):
            v = "" if val is None else str(val).strip()
            if api_name == "SOPT":
                v = pad_sopt(v)
            rec[api_name] = v
        records.append(normalise_record(rec))

    print(f"  ✅  {len(records)} sort option records read from {SOURCE_TABLE} (SOURCE).")
    return records
...
def sync_view_file_sort() -> None:
    ...
    # ------------------------------------------------------------------ #
    # Step 1 – SOURCE: local doppio.db (CSYSOR)                           #
    # ------------------------------------------------------------------ #
    print(f"\n🔍  Step 1 – Reading {SOURCE_TABLE} from {DEFAULT_DB_PATH} (SOURCE) …")
    source_all = list_all_srt_opts_db(DEFAULT_DB_PATH)
    ...
    # ------------------------------------------------------------------ #
    print(f"\n{'═' * 60}")
    print(f"  CHC_SyncSortingOptions – Run Summary")
    print(f"{'═' * 60}")
