...
POLL_SECONDS = 5   # how often to scan the input folder
...
def fetch_csytab(
    ...
    EXPORTMI QERY used:
        CTSTCO,CTSTKY,CTLNCD,CTTX40,CTTX15,CTPARM from CSYTAB
        where CTSTCO = <stco_code>

    CTPARM is parsed according to the 'sDS<stco_code>' data structure.

    The returned records use 4-char short-name keys (STCO, STKY, LNCD, TX40,
    TX15, plus the parsed PARM sub-fields).  CTPARM is not included.

    Returns [] on failure or when no records are found.
    """
    label = f"CSYTAB/{stco_code}"
    qery  = (
        f"CTSTCO,CTSTKY,CTLNCD,CTTX40,CTTX15,CTPARM "
...
def process_workbook(wb_path: Path, session: requests.Session) -> None:
    ...
    for sheet_name in sheets[2:]:          # tabs 1 & 2 skipped
        ...
        # ── 7. Identify missing records (shared by both paths) ────────────────
        ...
        if not missing:
            print("    ✅  All M3 records are already present.")
            continue

        # ── 8. Write missing rows ─────────────────────────────────────────
...
def monitor_input_folder(session: requests.Session) -> None:
    """
    Polls INPUT_FOLDER every POLL_SECONDS seconds for new .xlsx / .xlsm files.
    Each file is processed once and then remembered so it is not re-processed.
    ...
    print(f"  ℹ️   Drop Excel workbooks into that folder to process them.")
    ...
    while True:
        new_files = sorted(
            ...
            if f.suffix.lower() in (".xlsx", ".xlsm") and f.name not in processed
        )

        if new_files:
            for wb_path in new_files:
                try:
                    process_workbook(wb_path, session)
                except Exception as exc:
                    print(f"  ❌  Unexpected error processing {wb_path.name}: {exc}")
                finally:
                    processed.add(wb_path.name)
        else:
            print(
                f"  ⏳  Waiting for workbooks …  "
