...
def fetch_orders(db_path):
    """Fetch ORNO and PONR from 'fixes' table."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("""
        SELECT ORNO, PONR, CUOR FROM fixes ORDER BY ORNO, PONR
    """)
...
def ensure_text_id(order_no, order_line, customer_order, session):
    ...
    payload = {
        ...
        "transactions": [
            {
                "transaction": "ChgLineBatchEnt",
                "record": {
                    "ORNO": str(order_no),
                    "PONR": str(order_line),
                    "CUOR": str(customer_order)
                }
            }
        ]
    }
    ...
    """Return TXID for an order/line, creating if necessary."""
    # Step 1: GetTextID
    payload = {
        "program": "CRS980MI",
        "transactions": [
        ...
        "transactions": [
            {
                ...
                "record": {
                    ...
                    "LNCD": "GB",
                    "LINO": "1",
                    "TX60": "Canceled on October 19/20, 2025 (2025-10-19/20) for the"
                }
            }
        ]
    }
...
def run_paccar():
    ...
    with requests.Session() as session:
        ...
        for order_no, order_line, customer_order in tqdm(orders, desc="Processing orders"):
            try:
                txid = ensure_text_id(order_no, order_line, customer_order, session)
                add_text_blocks(txid, session)
            except Exception as e:
                tqdm.write(f"❌ Failed on order {order_no}-{order_line}: {e}")
