...
Public Function ExecuteApiCall(request As ApiRequest) As apiResponse
    ...
    If Not DoppioAuth.EnsureAuthenticated() Then
    ...
    If request.UseCache And request.cacheKey <> "" Then
        If DoppioCache.Cache_TryGetFromCache(request.cacheKey, response) Then
            ExecuteApiCall = response
            Exit Function
        End If
    End If
    ...
    Do While retryCount < maxRetries
        httpResponse = DoppioHttp.ExecuteRequest(httpConfig)
        ...
        ElseIf httpResponse.IsUnauthorized Then
            ...
            If DoppioAuth.HandleUnauthorized() Then
                ...
                httpConfig.authHeader = DoppioConfig.Config_GetAuthorizationHeader()
    ...
    If response.success And request.UseCache And request.cacheKey <> "" Then
        DoppioCache.Cache_StoreInCache request.cacheKey, response
    End If
...
Public Function ExecuteMiGet(program As String, transaction As String, _
    ...
    With request
        .mainUrl = DoppioConfig.Config_MainUrl
        .apiPath = MI_API_PATH
        .endpoint = endpoint
        .body = ""
        .apiType = ApiType_MI
...
Public Function ExecuteMiBulk(program As String, transactions As String) As apiResponse
    ...
    ' Build the bulk endpoint URL with parameters
    endpoint = "?maxrecs=" & settings.MaxRecords & "&extendedresult=true"
    
    If DoppioConfig.Config_M3User <> "" Then
        endpoint = endpoint & "&m3user=" & DoppioConfig.Config_M3User
    End If
    
    If settings.righttrim Then
        endpoint = endpoint & "&righttrim=true"
    Else
        endpoint = endpoint & "&righttrim=false"
    End If
    
    If DoppioConfig.Config_Company <> "" Then
        endpoint = endpoint & "&cono=" & DoppioConfig.Config_Company
    End If
    
    If DoppioConfig.Config_Division <> "" Then
        endpoint = endpoint & "&divi=" & DoppioConfig.Config_Division
    End If
...
Private Function BuildHttpConfigForApi(request As ApiRequest) As httpConfig
    ...
    With config
        ...
        .authHeader = DoppioConfig.Config_GetAuthorizationHeader()
        .body = request.body
        .TimeoutSeconds = DoppioConfig.Config_MaxTimeout
        .WriteToFile = True
    End With
...
Private Function ParseMiResponse(responseBody As String) As apiResponse
    ...
    Set json = JsonConverter.ParseJson(responseBody)
...
''
Public Function BuildBulkTransactions(program As String, method As String, _
    ...
    BuildBulkTransactions = transactions
    Exit Function
