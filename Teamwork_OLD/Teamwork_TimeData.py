"""
Teamwork_TimeData.py
--------------------
Fetch Teamwork time entries and save them to BigQuery.

BigQuery: spatial-earth-492100-b7.teamwork.TimeData
"""
...
def _init_bq_tables(client):
    ...
    sync_table = bigquery.Table(
        f"{BQ_PROJECT}.{BQ_DATASET}.SyncState",
        schema=[
            bigquery.SchemaField("name",      "STRING"),
            bigquery.SchemaField("last_sync", "STRING"),
        ],
    )
    client.create_table(sync_table, exists_ok=True)


def _get_last_sync_bq(client):
    try:
        rows = [dict(r) for r in client.query(
            f"SELECT last_sync FROM `{BQ_PROJECT}.{BQ_DATASET}.SyncState` "
            f"WHERE name = 'TimeData' LIMIT 1"
        ).result()]
        return str(rows[0]['last_sync']) if rows else None
    except Exception:
        return None
...
def _update_sync_state_bq(client, timestamp_str: str):
    from google.cloud import bigquery
    client.query(
        f"DELETE FROM `{BQ_PROJECT}.{BQ_DATASET}.SyncState` WHERE name = 'TimeData'"
    ).result()
    ...
    client.load_table_from_json(
        [{'name': 'TimeData', 'last_sync': timestamp_str}],
        f"{BQ_PROJECT}.{BQ_DATASET}.SyncState",
        job_config=job_config,
    ).result()


def _write_to_bq(client, df: pd.DataFrame, updated_after):
    from google.cloud import bigquery
    ...
    ndjson_bytes = io.BytesIO(
        '\n'.join(json.dumps(r) for r in records).encode('utf-8')
    )

    if updated_after:
        updated_ids = ', '.join(f"'{i}'" for i in df['id'].dropna().astype(str).tolist())
        if updated_ids:
            client.query(
                f"DELETE FROM `{table_id}` WHERE CAST(id AS STRING) IN ({updated_ids})"
            ).result()
        job_config = bigquery.LoadJobConfig(
            write_disposition=bigquery.WriteDisposition.WRITE_APPEND,
            source_format=bigquery.SourceFormat.NEWLINE_DELIMITED_JSON,
    ...
    client.load_table_from_file(ndjson_bytes, table_id, job_config=job_config).result()
...
def start_function():
    ...
    _init_bq_tables(client)
    last_sync = _get_last_sync_bq(client)
    ...
    if all_dfs:
        print("Consolidating data and writing to BigQuery...")
        new_df = pd.concat(all_dfs, ignore_index=True)
        bq_df = new_df.rename(columns=COLUMN_RENAME)

        end_timestamp = datetime.now()
        ts_str = end_timestamp.strftime("%Y-%m-%d %H:%M:%S")

        _write_to_bq(client, bq_df, updated_after)
        _update_sync_state_bq(client, ts_str)
        print(f"Successfully written {len(bq_df)} rows to BigQuery {BQ_PROJECT}.{BQ_DATASET}.{TABLE_NAME}")
    else:
