...
- m3Tables.xlsx
- m3TableCols.xlsx
- m3DataStructures.xlsx
- m3ReferenceFields.xlsx
...
extract_rules = pd.read_excel(get_path('ExtractRules.xlsx'))
m3_programs = pd.read_excel(get_path('m3Programs.xlsx'))
m3_tables = pd.read_excel(get_path('m3Tables.xlsx'))
m3_table_cols = pd.read_excel(get_path('m3TableCols.xlsx'))
m3_data_structures = pd.read_excel(get_path('m3DataStructures.xlsx'))
m3_reference_fields = pd.read_excel(get_path('m3ReferenceFields.xlsx'))
...
# Merge ExtractRules with m3Programs to get program descriptions and DataStructure
merged = extract_rules.merge(m3_programs, left_on='Pgm', right_on='programName', how='left')
merged['Tbl_Short'] = merged['Tbl'].str[:6]

# Merge with m3Tables to get table information
merged = merged.merge(m3_tables, left_on='Tbl_Short', right_on='tableName', how='left')
merged['tableLongName'] = merged['Tbl']
merged['tableDescription'] = merged['tableDescription'].apply(
    lambda x: str(x)[4:44] if pd.notna(x) and len(str(x)) > 4 else ''
)

query_result = merged[['Area', 'Pgm', 'programDescription', 'Tbl_Short', 'DataStructure', 'tableLongName', 'tableDescription']].copy()
...
for area in areas:
    ...
    for _, row in unique_programs.iterrows():
        ...
        table_long_name = str(row.get('tableLongName', '')) if pd.notna(row.get('tableLongName')) else ''
        table_desc = str(row.get('tableDescription', '')).upper() if pd.notna(row.get('tableDescription')) else ''
        if table_long_name or table_desc:
            pgm_sheet['A3'] = f"({table_long_name}) {table_desc}"
            pgm_sheet['A3'].font = Font(bold=True, size=12)
