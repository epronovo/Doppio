...
from ADP_Concur_Result import (
    ADP_Concur_clear_results,
    ADP_Concur_load_result,
    ADP_Concur_result_people,
    ADP_Concur_result_summary,
    ADP_Concur_runs,
    looks_like_a_result,
    xls_supported,
)
...
def api_upload():
    ...
    for fs in files:
        ...
        if not name.lower().endswith((".xlsx", ".xlsm", ".xls")):
            errors.append({"file": name,
                           "message": "Not a workbook - expected .xlsx, or "
                                      ".xls for a Concur result."})
            continue
        dest = UPLOAD_DIR / name
        fs.save(dest)
        # One drop zone, two kinds of file. Which one it is is decided by
        # what is inside it, not by the name: Concur's result files are named
        # 'Employee_p0010945e24e Run-18.xls' and nobody keeps that.
        try:
            if looks_like_a_result(dest):
                loads.append(ADP_Concur_load_result(conn, dest))
            else:
    ...
    if not (results or loads):
        return jsonify(
            status="error", results=results, loads=loads, errors=errors,
            message="; ".join(f"{e['file']}: {e['message']}" for e in errors)
                    or "Nothing could be loaded from those files."), 400
    return jsonify(status="success", results=results, loads=loads, errors=errors)
...
def _employee_filter() -> tuple[str, list]:
    ...
    if source in ("adp", "manual", "workbook"):
...
def api_export_all():
    ...
    body = request.get_json(force=True, silent=True) or {}
    try:
        files = ADP_Concur_export_all(conn, load_config(),
...
def api_clear():
    ...
    if what == "employees":
        ...
        # Same rule as the sources above: absent means the usual thing, an
        # empty list means the caller named nothing and must not fall through
        # to a default that deletes everything.
        parts = body.get("parts")
        if parts is None:
            parts = list(HISTORY_PARTS)
        try:
            out = clear_history(conn, parts)
        except ValueError as exc:
            return jsonify(status="error", message=str(exc)), 400
    else:
...
def api_clear_preview():
    ...
    return jsonify(status="success", history=history_preview(db()),
                   **clear_preview(db(), sources))
...
def api_results():
    """
    One load from Concur, grouped by cause, with the people named.

    'view=people' turns the same load inside out: who to go and fix, worst
    first. Both come from the same rows, because a result is only useful in
    those two shapes - what is broken, and who is broken.
    """
    conn = db()
    load_key = request.args.get("load_key", type=int)
    if (request.args.get("view") or "") == "people":
        return jsonify(status="success",
                       people=ADP_Concur_result_people(conn, load_key),
                       runs=ADP_Concur_runs(conn))
    return jsonify(status="success", runs=ADP_Concur_runs(conn),
                   **ADP_Concur_result_summary(conn, load_key))


@app.route("/api/results/<int:load_key>", methods=["DELETE"])
def api_results_delete(load_key: int):
    return jsonify(status="success", **ADP_Concur_clear_results(db(), load_key))


@app.route("/api/results/pick", methods=["POST"])
def api_results_pick():
    """
    Turn a load result into a selection.

    The point of reading a result is doing something about it, and the thing
    you almost always want to do is look at exactly those people. This takes a
    category - or the whole load - and picks everybody in it, so the Employees
    and Hierarchy tabs narrow to the ones Concur complained about.
    """
    body = request.get_json(silent=True) or {}
    load_key = body.get("load_key")
    category = (body.get("category") or "").strip()
    level = (body.get("level") or "").strip()
    conn = db()

    sql = ("SELECT DISTINCT employee_key FROM ADP_Concur_Results "
           "WHERE employee_key IS NOT NULL")
    args: list = []
    if load_key:
        sql += " AND load_key = ?"
        args.append(load_key)
    if category:
        sql += " AND category = ?"
    ...
    # Replaces the selection rather than adding to it: "show me these people"
    # means these people, not these on top of whatever was picked last week.
    selection_clear(conn)
    selection_add(conn, keys, reason=f"Concur result: {category or 'whole load'}")
    return jsonify(status="success", picked=len(keys),
                   **selection_summary(conn))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not xls_supported():
        log.warning("Concur result files (.xls) cannot be read - "
                    "'pip install xlrd' to enable them.")
