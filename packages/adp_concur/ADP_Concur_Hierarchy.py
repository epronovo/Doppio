...
def ADP_Concur_subtree_keys(conn: sqlite3.Connection,
    ...
    themselves. What a pilot load scoped strictly downward resolves to
    before the extract is scoped by it.
    """
    keys: list[int] = []
    seen: set[int] = set()
...
def ADP_Concur_organisation_keys(conn: sqlite3.Connection,
                                 file_numbers: list[str]) -> list[int]:
    """
    Employee keys for one whole organisation: each person named, everyone
    under them, and everyone above them up to the top of the chain.

    What 'select this organisation' resolves to. The subtree alone would
    leave the organisation's own approval chain out of the selection - scope
    an extract to just the subtree and a 305/360 record can point at a
    supervisor_id who never made it into that file, which is exactly the
    'broken' chain ADP_Concur_chain_up warns about. Including the chain up
    keeps a selected organisation self-contained.
    """
    keys: list[int] = []
    seen: set[int] = set()
    for fn in file_numbers:
        for row in ADP_Concur_subtree(conn, fn):
            if row["employee_key"] not in seen:
                seen.add(row["employee_key"])
                keys.append(row["employee_key"])
        for row in ADP_Concur_chain_up(conn, fn)["chain"]:
            if row["employee_key"] not in seen:
                seen.add(row["employee_key"])
                keys.append(row["employee_key"])
    return keys
