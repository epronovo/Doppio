...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Mimic the real 403: a cono the service account cannot use is
        # rejected before any transaction runs.
        import urllib.parse as _up
        qs = _up.parse_qs(_up.urlparse(self.path).query)
        cono = (qs.get("cono") or [""])[0]
        if cono and cono not in ("100", "200"):
            return self._send({"error": f"no authority for company {cono}"}, 403)
        ...
        for t in payload["transactions"]:
            ...
            if program == "MNS405MI" and tx == "Lst":
                ...
                recs = list(USERS)
            elif program == "MNS095MI" and tx == "Lst":
                # a tenant that does NOT number its companies 001/100
                recs = [{"CONO": "100", "TX40": "Doppio Demo"},
                        {"CONO": "200", "TX40": "Doppio Test"}]
            elif program == "MNS100MI" and tx == "LstDivisions":
                cono = (rec.get("CONO") or "").strip()
                divs = {"100": [{"DIVI": "100", "TX15": "Demo main"},
                                {"DIVI": "200", "TX15": "Demo second"}],
                        "200": [{"DIVI": "100", "TX15": "Test main"}]}
                recs = [{**d, "CONO": cono} for d in divs.get(cono, [])]
            else:
